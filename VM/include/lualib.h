...
LUALIB_API int fieldargs(lua_State* L, int farg, int* width);
LUALIB_API int b_shift(lua_State* L, unsigned int r, int i);
...
LUALIB_API int interruptThread(lua_State* L, lua_State* co);
...
// lmathlib.cpp
LUALIB_API uint32_t pcg32_random(uint64_t* state);
LUALIB_API void pcg32_seed(uint64_t* state, uint64_t seed);

LUALIB_API inline float perlin_fade(float t);
LUALIB_API inline float perlin_lerp(float t, float a, float b);
LUALIB_API inline float perlin_grad(int hash, float x, float y, float z);
LUALIB_API float perlin(float x, float y, float z);
